...
Today I will be talking about the joint work of my advisor's and mine, it's "Syntax-generic Operations, Reflectively Reified".}
...
\note{Then let's define some operations on this syntax. Such operations are scope-safe. For example, let's define a rename function. A \mi{rename} function says that if there's a mapping from every variable in a context $\varGamma$ to the other context $\varDelta$, we can always find a mapping from every term that is typed in $\varGamma$ to a term typed in $\varDelta$. Because we can remap every variable in that term. If we encounter a subterm that has a extended context, for example, the case of lambda abstraction, the variable mapping should be extended accordingly.}
