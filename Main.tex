...
\newunicodechar{·}{\ensuremath{\cdotp}}
...
\emph{Intrinsic typing} is a common pattern that dependently typed programmers use to define lambda calculus with De Bruijn indices.
\metroset{block=fill}
...
\>[B]{}\hsindent{3}{}\<[3]%
\>[3]{}\Varid{con}\;{}\<[11]%
\>[11]{}\mathbin{:}\;\Conid{ℕ}\;\Varid{→}\;\Conid{Γ}\;\Varid{⊢}\;\Conid{Nat}{}\<[E]%
\\
\>[B]{}\hsindent{3}{}\<[3]%
\>[3]{}\Varid{\char95 `∗\char95 }\;{}\<[11]%
\>[11]{}\mathbin{:}\;\Conid{Γ}\;\Varid{⊢}\;\Conid{Nat}\;\Varid{→}\;\Conid{Γ}\;\Varid{⊢}\;\Conid{Nat}\;\Varid{→}\;\Conid{Γ}\;\Varid{⊢}\;\Conid{Nat}{}\<[E]%
\\
\>[B]{}\hsindent{3}{}\<[3]%
\>[3]{}\Varid{`let}\;{}\<[11]%
\>[11]{}\mathbin{:}\;\Conid{Γ}\;\Varid{⊢}\;\Conid{A}\;\Varid{→}\;\Conid{Γ}\;\Varid{,}\;\Conid{A}\;\Varid{⊢}\;\Conid{B}\;\Varid{→}\;\Conid{Γ}\;\Varid{⊢}\;\Conid{B}{}\<[E]%
\\
\>[B]{}\hsindent{3}{}\<[3]%
\>[3]{}\Varid{`⟨\char95 ,\char95 ⟩}\;{}\<[11]%
\>[11]{}\mathbin{:}\;\Conid{Γ}\;\Varid{⊢}\;\Conid{A}\;\Varid{→}\;\Conid{Γ}\;\Varid{⊢}\;\Conid{B}\;\Varid{→}\;\Conid{Γ}\;\Varid{⊢}\;\Conid{A}\;\Varid{`×}\;\Conid{B}{}\<[E]%
\\
\>[B]{}\hsindent{3}{}\<[3]%
\>[3]{}\Varid{`proj₁}\;{}\<[11]%
\>[11]{}\mathbin{:}\;\Conid{Γ}\;\Varid{⊢}\;\Conid{A}\;\Varid{`×}\;\Conid{B}\;\Varid{→}\;\Conid{Γ}\;\Varid{⊢}\;\Conid{A}{}\<[E]%
\\
\>[B]{}\hsindent{3}{}\<[3]%
\>[3]{}\Varid{`proj₂}\;{}\<[11]%
\>[11]{}\mathbin{:}\;\Conid{Γ}\;\Varid{⊢}\;\Conid{A}\;\Varid{`×}\;\Conid{B}\;\Varid{→}\;\Conid{Γ}\;\Varid{⊢}\;\Conid{B}{}\<[E]%
\\
\>[B]{}\hsindent{3}{}\<[3]%
\>[3]{}\Varid{case×}\;{}\<[11]%
\>[11]{}\mathbin{:}\;\Conid{Γ}\;\Varid{⊢}\;\Conid{A}\;\Varid{`×}\;\Conid{B}\;\Varid{→}\;\Conid{Γ}\;\Varid{,}\;\Conid{A}\;\Varid{,}\;\Conid{B}\;\Varid{⊢}\;\Conid{C}\;\Varid{→}\;\Conid{Γ}\;\Varid{⊢}\;\Conid{C}{}\<[E]%
\ColumnHook
\end{hscode}\resethooks
	\end{exampleblock}
\end{frame}
...
\begin{frame}[fragile]{Existing work by Allais et al.}
	Allais et al. present a universe of syntaxes $\mathit{Desc}$,
	\begin{hscode}\SaveRestoreHook
\column{B}{@{}>{\hspre}l<{\hspost}@{}}%
\column{9}{@{}>{\hspre}l<{\hspost}@{}}%
...
\ColumnHook
\end{hscode}\resethooks
	can be encoded in $\mathit{Desc}$:
\end{frame}

\begin{frame}[fragile]{Existing work by Allais et al.}
	The universe Desc is specific about what syntaxes are captured:
\end{frame}

\begin{frame}[fragile]{Existing work by Allais et al.}
	Generic programs are defined as Semantics records,
	A generic program is realized via semantics on the fixpoint constructor Tm.
	
\end{frame}

\begin{frame}[fragile]{Motivation cont.}
	The problem with applying standard datatype-generic programming to syntax-generic operations is..
	\begin{itemize}
		\item The datatype to work on is non-intuitive
		\item The function definition to work on is non-intuitive
		\item Require the programmer to understand the syntax universe
...
We define Syntax, a predicate on DataD, to capture Agda datatypes that are syntaxes in Allais et al.'s Desc universe. Allais et al.'s universe cover specific syntaxes that
	\begin{itemize}
		\item each has a variable rule as its first rules,
		\item are not universe polymorphic,
		\item have two indexes, one of them being a list, implying single context, and
		\item only allow context extentions in subterms.
	\end{itemize}
\end{frame}

\begin{frame}[fragile]{The Syntax Predicate}
	\metroset{block=fill}
	\begin{exampleblock}{A complete Syntax proof}
		\begin{hscode}\SaveRestoreHook
\column{B}{@{}>{\hspre}l<{\hspost}@{}}%
\column{9}{@{}>{\hspre}l<{\hspost}@{}}%
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
\>[B]{}\Conid{SyntaxPCF}\;\mathbin{:}\;\Conid{Syntaxᵈ}\;\Conid{Type}\;(\Varid{findDataD}\;(\Varid{quote}\;\Conid{PCF})){}\<[E]%
\\
...
\\
\>[B]{}\hsindent{9}{}\<[9]%
\>[9]{}\Varid{,}\;(\Varid{refl}\;\Varid{,ωω}\;\Varid{refl})\;{}\<[E]%
\\
\>[B]{}\hsindent{9}{}\<[9]%
\>[9]{}\Varid{,}\;\anonymous \;{}\<[E]%
\\
